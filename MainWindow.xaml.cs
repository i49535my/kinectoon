...
namespace ShapeGame
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private double[] Taiko1_X = new double[] { -0.9, -0.2 };
        //                                 _____________
        //  Taiko1 : 和太鼓   (-0.9,-0.10) |            | (-0.2,-0.10)
        //　                  (-0.9,-0.86) |____________| (-0.2,-0.86)

        private double[] Taiko1_Y = new double[] { -0.10, -0.86 };  
        
        private double[] Taiko2_X = new double[] { 0.016, 0.70 };
        //                                  _____________
        //  Taiko2 : ドラム   (0.016,-0.10) |            | (0.70,-0.10)
        //　                  (0.016,-0.86) |____________| (0.70,-0.86)

        private double[] Taiko2_Y = new double[] { -0.10, -0.86 };
        ...
        private int z = 0;          //右手が頭より上だと1
        //現在の画像のパス名
        static public string path = @"C:\H28創造工学\ShapeGame\bin\Debug\太鼓.bmp";

        static public int END = 0;  //CAPTUREボタンで画像を変えるための変数
        static public int A;        //MENUフォームが一度しか開かないようにするための変数
        static public int X = 1;    //画像の名前の数字カウント変数
        static public string[] EXAMPLE1 = new string[] {"仮"};      //画像の名前1
        static public string[] EXAMPLE2 = new string[] { "かり" };  //画像の名前2
        static public int FIRST = 0;         //初期状態に戻すボタンで画像を変えるための変数
        static public int TUUKAN = 0;        //一つ前に戻るボタンで画像を変えるための変数
        static public string picadd = path; //前の画像の保存先
        ...
        public MainWindow()
        {
            ...
            this.DataContext = new { X = path, Y = this.KinectSensorManager };
        ...
        {
            if (END == -1 | FIRST == -1 | TUUKAN == -1)  //表示する画像の変更があった際に入る
            {
                this.DataContext = new { X = path, Y = this.KinectSensorManager };  //XAMLに画像データを送る
                if (END == -1) END = 0;                                             //リセット
                else if (FIRST == -1) FIRST = 0;                                    //   〃
                else if (TUUKAN == -1) TUUKAN = 0;                                  //   〃
            }
